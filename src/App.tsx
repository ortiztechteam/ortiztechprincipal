...
import arenaPalmaresImage from './assets/img/ArenaPalmares.png';
import ballBurgerImage from './assets/img/BallBurger.png';
import contabilidadeImage from './assets/img/contabilidade.png';
import pointSucosImage from './assets/img/PointSucos.png';
...
const whatsappUrl = 'https://wa.me/5531984350134?text=Ol%C3%A1%2C%20quero%20criar%20um%20site%20para%20meu%20neg%C3%B3cio.';

const projects: Project[] = [
  { title: 'Arena Palmares', category: 'Esporte & lazer', description: 'Uma presença vibrante para apresentar a arena e tornar a reserva de quadras mais simples.', image: arenaPalmaresImage },
  { title: 'Point Sucos', category: 'Gastronomia', description: 'Uma experiência leve e cheia de sabor para mostrar o cardápio e aproximar clientes da marca.', image: pointSucosImage },
  { title: 'Nexus Contabilidade', category: 'Serviços financeiros', description: 'Uma comunicação clara e confiável para transformar contabilidade em parceria para o negócio.', image: contabilidadeImage },
  { title: 'Ball Burguer', category: 'Gastronomia', description: 'Um cardápio digital marcante, feito para abrir o apetite e facilitar cada pedido.', image: ballBurgerImage },
];
...
function BrowserPreview({ project }: { project: Project }) {
  return <div className="browser-preview project-preview"><img src={project.image} alt={`Página inicial do projeto ${project.title}`} /></div>;
}
...
function Home() {
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <section className="section showcase-section"><div className="container"><div className="section-heading centered"><SectionEyebrow>Trabalho recente</SectionEyebrow><h2>Feito para <em>ser lembrado.</em></h2><p>Projetos com propósito, personalidade e resultados reais.</p></div><div className="featured-project"><BrowserPreview project={projects[0]} /><div className="featured-info"><span className="project-index">01 / 04</span><h3>Arena Palmares<br /><em>o jogo começa aqui.</em></h3><p>{projects[0].description}</p><div className="tag-list"><span>Estratégia</span><span>UX/UI Design</span><span>Desenvolvimento</span></div><a href="/portfolio" onClick={(event) => { event.preventDefault(); window.history.pushState({}, '', '/portfolio'); window.dispatchEvent(new PopStateEvent('popstate')); }} className="arrow-link">Ver projeto completo <ArrowRight size={16} /></a></div></div><a href="/portfolio" className="center-link" onClick={(event) => { event.preventDefault(); window.history.pushState({}, '', '/portfolio'); window.dispatchEvent(new PopStateEvent('popstate')); }}>Explorar portfólio <ArrowRight size={16} /></a></div></section>
...
function Footer() { return <footer><div className="container footer-main"><div><Logo /><p>Sites com intenção<br />para negócios com visão.</p></div><div className="footer-links"><div><span>Explore</span><a href="#servicos">Serviços</a><a href="/portfolio">Portfólio</a><a href="#processo">Processo</a></div><div><span>Fale com a gente</span><a href={whatsappUrl} target="_blank" rel="noreferrer">WhatsApp <ExternalLink size={12} /></a><a href="mailto:oi@ortiztech.com.br">oi@ortiztech.com.br</a></div></div></div><div className="container footer-bottom"><span>© 2026 Ortiz Tech. Feito com intenção.</span><div><a href="https://www.instagram.com/ortiztechbr/" target="_blank" rel="noreferrer"><Instagram size={16} /></a><a href={whatsappUrl} target="_blank" rel="noreferrer"><MessageCircle size={16} /></a></div></div></footer>; }

function Portfolio() { return <div className="app-shell portfolio-page"><Nav portfolio /><main><section className="portfolio-hero"><div className="container"><SectionEyebrow>Projetos selecionados</SectionEyebrow><h1>Ideias que<br /><em>ganham forma.</em></h1><p>Uma seleção de trabalhos feitos para marcas que entendem que presença é tudo.</p></div></section><section className="portfolio-grid-section"><div className="container"><div className="portfolio-toolbar"><span>04 projetos</span><span className="toolbar-line" /><span>2026</span></div><div className="portfolio-grid">{projects.map((project, index) => <article className={`portfolio-card portfolio-card-${index % 2 === 0 ? 'tall' : 'standard'}`} key={project.title}><BrowserPreview project={project} /><div className="portfolio-card-info"><div><h2>{project.title}</h2><p>{project.category}</p><small>{project.description}</small></div><ArrowUpRightIcon /></div></article>)}</div></div></section><CTASection /></main><Footer /></div>; }
